...
前面所罗列的userCF, itemCF, personal rank都属于基于邻域的召回。除了基于邻域的召回之外，还有基于内容特征的召回，即content-based recommendation。基于内容的召回的思路是：对user和item分别做画像，离线建立user->feature正排表和feature->item倒排表，然后放入内存或数据库，在线构造user->item形成推荐结果。对user的画像要考虑时间衰减。对item的画像，主要包括topic和genre的finding，给item打上标签。以文本类的item为例，包括词库表建立，命名实体识别，关键词排序等等。

用户的推荐结果不受别的用户的影响。

工业界通常采用多种召回算法并存的架构，对多种算法的推荐结果进行在线merge，并排序。
